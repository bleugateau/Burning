...
namespace Burning.DofusProtocol.Network.Messages
{
  public class ObjectDropMessage : NetworkMessage
  {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.objectUID < 0U)
        throw new Exception("Forbidden value (" + (object) this.objectUID + ") on element of ObjectDropMessage.objectUID.");
      this.quantity = reader.ReadVarUhInt();
      if (this.quantity < 0U)
        throw new Exception("Forbidden value (" + (object) this.quantity + ") on element of ObjectDropMessage.quantity.");
    }
  }
}
