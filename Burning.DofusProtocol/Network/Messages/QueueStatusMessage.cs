...
namespace Burning.DofusProtocol.Network.Messages
{
  public class QueueStatusMessage : NetworkMessage
  {
    ...
    public QueueStatusMessage(uint position, uint total)
    {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.position < 0U || this.position > (uint) ushort.MaxValue)
        throw new Exception("Forbidden value (" + (object) this.position + ") on element of QueueStatusMessage.position.");
      this.total = (uint) reader.ReadUShort();
      if (this.total < 0U || this.total > (uint) ushort.MaxValue)
        throw new Exception("Forbidden value (" + (object) this.total + ") on element of QueueStatusMessage.total.");
    }
  }
}
