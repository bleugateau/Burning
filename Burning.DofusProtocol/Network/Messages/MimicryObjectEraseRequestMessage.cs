...
namespace Burning.DofusProtocol.Network.Messages
{
  public class MimicryObjectEraseRequestMessage : NetworkMessage
  {
    ...
    public MimicryObjectEraseRequestMessage(uint hostUID, uint hostPos)
    {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.hostUID < 0U)
        throw new Exception("Forbidden value (" + (object) this.hostUID + ") on element of MimicryObjectEraseRequestMessage.hostUID.");
      this.hostPos = (uint) reader.ReadByte();
      if (this.hostPos < 0U || this.hostPos > (uint) byte.MaxValue)
        throw new Exception("Forbidden value (" + (object) this.hostPos + ") on element of MimicryObjectEraseRequestMessage.hostPos.");
    }
  }
}
