...
namespace Burning.DofusProtocol.Network.Messages
{
  public class PrismFightAttackerRemoveMessage : NetworkMessage
  {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.subAreaId < 0U)
        throw new Exception("Forbidden value (" + (object) this.subAreaId + ") on element of PrismFightAttackerRemoveMessage.subAreaId.");
      this.fightId = (uint) reader.ReadVarUhShort();
      if (this.fightId < 0U)
        throw new Exception("Forbidden value (" + (object) this.fightId + ") on element of PrismFightAttackerRemoveMessage.fightId.");
      this.fighterToRemoveId = (double) reader.ReadVarUhLong();
      if (this.fighterToRemoveId < 0.0 || this.fighterToRemoveId > 9.00719925474099E+15)
        throw new Exception("Forbidden value (" + (object) this.fighterToRemoveId + ") on element of PrismFightAttackerRemoveMessage.fighterToRemoveId.");
    }
  }
}
