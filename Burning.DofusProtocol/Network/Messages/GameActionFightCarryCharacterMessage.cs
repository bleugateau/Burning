...
namespace Burning.DofusProtocol.Network.Messages
{
  public class GameActionFightCarryCharacterMessage : AbstractGameActionMessage
  {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.targetId < -9.00719925474099E+15 || this.targetId > 9.00719925474099E+15)
        throw new Exception("Forbidden value (" + (object) this.targetId + ") on element of GameActionFightCarryCharacterMessage.targetId.");
      this.cellId = (int) reader.ReadShort();
      if (this.cellId < -1 || this.cellId > 559)
        throw new Exception("Forbidden value (" + (object) this.cellId + ") on element of GameActionFightCarryCharacterMessage.cellId.");
    }
  }
}
