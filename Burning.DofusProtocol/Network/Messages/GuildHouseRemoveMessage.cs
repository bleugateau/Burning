...
namespace Burning.DofusProtocol.Network.Messages
{
  public class GuildHouseRemoveMessage : NetworkMessage
  {
    ...
    public override void Deserialize(IDataReader reader)
    {
      ...
      if (this.houseId < 0U)
        throw new Exception("Forbidden value (" + (object) this.houseId + ") on element of GuildHouseRemoveMessage.houseId.");
      this.instanceId = (uint) reader.ReadInt();
      if (this.instanceId < 0U)
        throw new Exception("Forbidden value (" + (object) this.instanceId + ") on element of GuildHouseRemoveMessage.instanceId.");
